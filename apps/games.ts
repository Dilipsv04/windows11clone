...
// Data from: https://www.onlinegames.io/
export const GAMES_LIST: GameInfo[] = rawGameData.map(game => {
    ...
    return {
        id: game.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''),
        name: game.title.trim(),
        url: game.embed,
        imageUrl: game.image,
        description: game.description,
        category: tags[0].charAt(0).toUpperCase() + tags[0].slice(1) || 'Game',
        publisher: 'OnlineGames.io',
        rating: parseFloat((Math.random() * (4.8 - 3.5) + 3.5).toFixed(1)),
        ratingCount: `${(Math.random() * 5 + 1).toFixed(1)}k`,
        screenshots: [game.image],
        features: tags.slice(0, 4).map(tag => tag.charAt(0).toUpperCase() + tag.slice(1).replace('-', ' ')),
        releaseDate: `${randomMonth.toString().padStart(2, '0')}/${randomDay.toString().padStart(2, '0')}/${randomYear}`,
        lastUpdated: `05/${Math.floor(Math.random() * 28) + 1}/2024`,
        systemRequirements: 'This product should work on your device.',
        reviewSummary: `Users enjoy the addictive gameplay and ${tags[1] || 'fun'} mechanics. The game offers fun challenges, relaxation, and endless entertainment.`,
        reviewTags: [tags[2] || 'Fun', tags[3] || 'Casual', tags[4] || 'Free'].map(tag => tag.charAt(0).toUpperCase() + tag.slice(1)),
    };
});
